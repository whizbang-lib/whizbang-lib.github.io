...
export class PatternsMarkdownPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.url.subscribe(async segments => {
      ...
      // Generate breadcrumbs for the current path (add 'patterns/' prefix since we're in the patterns route)
      const breadcrumbPath = fullPath ? `patterns/${fullPath}` : 'patterns';
  ...
  private async resolveMarkdownPath(urlPath: string): Promise<string> {
    // Simplified path resolution for patterns - just use the URL path directly
    // For patterns, we don't need versioning logic
    if (!urlPath || urlPath === '') {
      return 'assets/patterns/overview.md';
    }
    
    // Map the URL path directly to the patterns directory
    return `assets/patterns/${urlPath}-pattern.md`;
  }
  ...
  private loadMarkdownContent(path: string) {
    this.http.get(path, { responseType: 'text' }).subscribe({
      next: (content) => {
        ...
        // Fix isolated markdown patterns (like bold text) that might have been disrupted by code block processing
        const fixedContent = this.processIsolatedMarkdown(finalContent);

        this.processedContent.set(fixedContent);
  ...
  /**
   * Process isolated markdown patterns that might have been disrupted by code block processing
   */
  private processIsolatedMarkdown(content: string): string {
    // Fix isolated bold text patterns that appear on their own lines
    // This handles cases where code block processing has left orphaned markdown syntax
    return content
      .replace(/^\*\*(.*?)\*\*$/gm, '<p><strong>$1</strong></p>')
      .replace(/\n\*\*(.*?)\*\*\n/g, '\n<p><strong>$1</strong></p>\n')
      // Also fix inline bold patterns that may have been disrupted
