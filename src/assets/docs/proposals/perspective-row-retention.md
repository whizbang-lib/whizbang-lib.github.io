...
- **Detection:** `GetByStreamIdAsync` finds no row, and the event store holds an earlier event **of a type this perspective folds** (`IEventStore.HasStreamEventsBeforeAsync(streamId, beforeEventId, handledEventTypes)`). The check only triggers on the row-miss branch, and it is perspective-aware on purpose: a physical stream shared by several contracts holds history the perspective never folded, and "any earlier event" read that as a reaped row on every first contact, re-folding with a full replay, a warning, and two system events written into the stream for its lifetime. The generated runner passes the same event-type list its `Apply` overloads use; the drivers match the stored `event_type` values through `EventTypeMatchingHelper`, never by a hand-built string. {verified: ResurrectionProbeIsPerspectiveAwareTests.FirstContact_OnStreamWithUnhandledHistory_CreatesRow_DoesNotRewindAsync, ResurrectionProbeIsPerspectiveAwareTests.ReapedRow_OnStreamWithHandledHistory_StillResurrectsAsync, EventStoreHistoryProbeSqlTests.TypedProbe_OlderEventsOfOtherTypesOnly_ReturnsFalseAsync, EventStoreHistoryProbeSqlTests.TypedProbe_MatchesStoredEventTypeFormat_NestedAndAssemblyQualifiedAsync, DapperEventStoreHistoryProbeTests.TypedProbe_OlderEventOfHandledType_ReturnsTrueAsync}
...
The declaration reaches the database by one key: `wh_perspective_registry.clr_type_name` holds the model's **CLR type name** (`Outer+Model` for a nested model class), written by the generator through `TypeNameUtilities.BuildClrTypeName` and looked up at startup through `TypeNameFormatter.FormatClrTypeName`. The two helpers are mirrors, pinned to each other by a contract test; a registry written under any other rendering matches nothing, and the sync warns (`RetentionDeclarationMatchedNoRegistryRow`) instead of discarding the zero-row result. Registries written under the earlier display-string form are adopted in place by `reconcile_perspective_registry` (reported as `renamed_key`), keyed on the table the service registered for the model. {verified: PerspectiveRegistryKeyTests.NestedModel_RegistryRowIsKeyedByTheClrFormAsync, PerspectiveRegistryKeyTests.DeclaredTtl_OnNestedModel_ReachesTheRegistryAsync, PerspectiveRegistryKeyTests.Sync_DeclarationMatchingNoRegistryRow_LogsAWarningNamingTheTypeAsync, ReconcilePerspectiveRegistryKeyAdoptionSqlTests.Reconcile_ExistingDisplayStringKey_IsAdoptedNotDuplicatedAsync, TypeNameMirrorContractTests.ClrTypeName_GeneratorAndRuntimeHelpers_AgreeOnEveryShapeAsync}
