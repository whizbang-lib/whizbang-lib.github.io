...
| Nothing knows what version anything is | `wh_schema_versions` is written on every initialization and never read. An instance cannot discover that the schema is newer than its own code — see [Rolling upgrades](#rolling-upgrades-two-versions-against-one-schema) |
| Reaping an instance does not stop it | `cleanup_stale_instances` deletes the row and releases the work; the instance's next heartbeat re-inserts it unguarded and it rejoins as though nothing happened |
...
Three steps are new relative to the shape the framework has today, each because real startup work currently has nowhere to live — and one existing step is drawn differently than before:
...
- **Not full version negotiation.** Declaring per-migration compatibility, and deciding what a consumer's own application version implies, is separate work. This proposal defines the seam it will need — `Assess`, and a capability model that can act on a verdict — and fixes the two version defects that exist today.
- **Not a way to avoid downtime on a breaking migration.** A breaking schema change is a planned outage. The handshake makes it bounded, announced and reversible; it does not make it free.
...
**Reaping an instance does not stop it.** `cleanup_stale_instances` deletes a stale instance's row and releases its work, but `register_instance_heartbeat` is an unguarded `INSERT … ON CONFLICT DO UPDATE`. The reaped instance's next heartbeat re-inserts it and it rejoins as though nothing had happened, still holding whatever it believed it owned. A pod paused by a long collection, a brief partition or a throttled node therefore returns and resumes against state that has moved on without it. The claim primitives are built to tolerate a good deal of this, so it is not necessarily corrupting today — but "we reaped it" currently means the record is gone, not that the process stopped, and every design above that treats reaping as a fence is relying on something the code does not do.
