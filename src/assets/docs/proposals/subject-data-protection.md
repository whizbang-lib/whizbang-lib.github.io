...
**The default is client-side: the DB never holds a key or sees a plaintext protected field.** That is SQL Server's *Always Encrypted* posture, and it is a security feature in its own right — the engine, its query logs, a DBA, a memory dump of the server, and every backup all stay blind to keys *and* plaintext. Performance comes from three levers, none of which weakens the erasure guarantee:
...
### Escape hatch: DB-side crypto (opt-in, Postgres-only)

If profiling ever shows the AES *CPU* (not key fetch) is a real bottleneck — unlikely with AES-NI — a Postgres-only **opt-in** mode can push the crypto into the engine via `pgcrypto`, caching the requested keys in an **`UNLOGGED`** table so they stay out of WAL, replicas, PITR, and base backups. This is a **deliberately weaker, ops-dependent** guarantee, documented as such: the engine now sees keys and plaintext (a larger trust surface — DBA, server memory, statement logs), and the cached key is still exposed to a **logical dump** (`pg_dump`) or a primary-disk forensic taken **during** its short TTL window. The authoritative key stays in the KMS and is still destroyed on erasure; the DB cache is ephemeral and evicted on the same `SubjectErased` signal. Unavailable on SQLite (no per-subject field crypto). **Not the default, and not recommended unless measured** — the client-side path is both easier to reason about and strictly safer.
