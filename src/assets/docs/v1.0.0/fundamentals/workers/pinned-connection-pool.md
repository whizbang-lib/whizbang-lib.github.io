...
On a measured production-shaped workload (production dev, alpha `0.674.1-alpha.32`, 16,759-event import), `DISCARD ALL` accounted for **22.5 % of total database time**. The pinned worker connection pool eliminates that overhead for the workers that actually drive the hot path.
...
- [production perf measurement (2026-06-10)](../../proposals/production-perf-baseline.md) — the measurement that motivated this feature.
