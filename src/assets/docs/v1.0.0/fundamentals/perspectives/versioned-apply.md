...
This race was the production `019f000e` saga's per-item strand on 2026-06-25 and survived into the v0.755 re-run on 2026-06-26 (2 of 350 items stranded at `Running` even though both their `SagaItemStartedEvent` and `SagaItemCompletedEvent` were durably committed to `wh_event_store`). The framework reconciler healed the saga, but the projection itself stayed wrong.
