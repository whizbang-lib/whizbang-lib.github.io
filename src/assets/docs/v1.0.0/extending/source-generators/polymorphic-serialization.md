...
:::updated
**The discriminator contract is fixed**: generated polymorphic factories always use the property name **`$type`**, and the value is the **simple type name** — except when two derived types under the same base share a simple name, in which case **the colliding types (and only those) use their fully qualified name** instead. Distinct simple names are never rewritten, so discriminators already written into stored payloads stay readable; a base with a collision previously could not be configured at all (System.Text.Json throws on duplicate discriminators, taking down polymorphism for the entire base), so no working wire format changes. The collision case is routine with sagas, whose generated event classes are named identically by construction — two `[Saga]` classes in one assembly collide on every event name. `[JsonPolymorphic]` on an abstract base is honored as a *discovery marker* (it triggers derived-type registration), but its `TypeDiscriminatorPropertyName` setting and custom `[JsonDerivedType]` string values are **ignored** by the generated `JsonTypeInfo` (`JsonContextSnippets.cs` — hardcoded `$type`; discriminator values come from `_assignPolymorphicDiscriminators`). If you need custom discriminators, that is currently unsupported — using them in attributes produces payloads that cannot be read back as typed events.
:::
