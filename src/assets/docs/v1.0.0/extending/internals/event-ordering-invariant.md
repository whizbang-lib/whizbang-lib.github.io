...
1. **Parallel emission inside one process.** A receptor running on N threads (`Parallel.ForEachAsync`, concurrent saga handlers) calls `TrackedGuid.NewMedo()` from each thread. The generator issues every id under one lock, so ids are in issue order across threads (see [How `NewMedo()` orders ids](/v1.0.0/fundamentals/identity/whizbang-ids#uuid7-generator)); what the threads cannot control is which of them reaches the lock first, so the ids follow issue order, not the order the threads' work logically happened in.
...
3. **Derived ids that drop the allocator's order.** An event a handler returns does not get a freshly issued id: its id is derived from the command that was handled, so a retry reproduces it (see [Emission identity](/v1.0.0/fundamentals/dispatcher/message-cascade#emission-identity)). A derivation is an allocator too, and it must keep the order of what it derives from. It keeps the source id's millisecond **and** the generator's counter, then the emission's ordinal. An earlier derivation kept the millisecond only, so events of two commands issued in the same millisecond sorted at random and could be versioned the wrong way round on their stream.
