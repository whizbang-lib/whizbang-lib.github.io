...
order: 1
description: >-
  Background worker for processing perspective checkpoints - automatic
  checkpoint creation, polling workflow, error tracking, and startup
  coordination
tags: >-
  perspective-worker, checkpoints, background-worker, lease-based-coordination,
  error-tracking
codeReferences:
...
**Claim**:
```sql{title="How Leases Work" description="How Leases Work" category="Implementation" difficulty="BEGINNER" tags=["Operations", "Workers", "Leases", "Work"]}
...
**Example**:
```csharp{title="BatchedCompletionStrategy (Default)" description="BatchedCompletionStrategy (Default)" category="Implementation" difficulty="BEGINNER" tags=["Operations", "Workers", "BatchedCompletionStrategy", "Default"]}
...
**Example**:
```csharp{title="InstantCompletionStrategy" description="InstantCompletionStrategy" category="Implementation" difficulty="INTERMEDIATE" tags=["Operations", "Workers", "InstantCompletionStrategy"]}
...
With `BatchedCompletionStrategy`, completions are deferred to the next poll cycle. During that window:

1. **Cycle N**: Events are processed, Apply is called, completions queued in memory
2. **Cycle N+1**: Completions from Cycle N are sent to SQL via `ProcessWorkBatchAsync`
3. **The gap**: Between Cycle N completing and Cycle N+1 sending completions, SQL can return the same events again
...
- **Retained**: After `MarkAsAcknowledged`, `ActivateRetention()` is called. The lease-aligned TTL (default 5 minutes) starts counting down from this moment.
- **Evicted**: After TTL expires, entries are removed. This correctly allows reprocessing for rewind/rebuild scenarios.
...
**Solution**:
```csharp{title="Problem: Checkpoints Not Processing" description="Problem: Checkpoints Not Processing" category="Implementation" difficulty="BEGINNER" tags=["Operations", "Workers", "Problem:", "Checkpoints"]}
...
2. Lease duration too short — events claimed by another instance before completion
...
**Solution**:
```sql{title="Problem: Failed Checkpoints Accumulating" description="Problem: Failed Checkpoints Accumulating" category="Implementation" difficulty="INTERMEDIATE" tags=["Operations", "Workers", "Problem:", "Failed"]}
