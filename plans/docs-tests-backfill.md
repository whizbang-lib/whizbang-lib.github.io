...
- **2026-08-05 — Phase 5 Track 1 COMPLETE** (library PRs #418, #419, #420, #421): four waves investigated all 355 uncovered symbols on published targets; **344 were tested-but-unlinked** and are now `<tests>`-tagged (~1,060 tag lines; stale/fabricated/misplaced tags cleaned along the way). Remaining uncovered on published pages: **62** — all confirmed Track-2 gaps (tests to write) or map-parser artifacts. Consolidated Track-2 write-list: PinnedPoolMetrics, CollectiveApplyBatchSize wiring, DapperCollectiveTableRegistry, WhizbangManagedHealthCheckExtensions, AddWhizbangSchemaReadyCheck, RebuildCommandReceptorRegistrar (hosted-service path), the EF Functions principal-filtering surface (JsonArrayContainsAnyTranslator + plugin + WhizbangJsonDbFunctions), StuckRowSentinel{MaxAttempts,Limit} propagation, PerspectiveRewindOptions debounce windows, ImmediateDetachedChainWarningThreshold, dead contract surface (ILifecyclePerspectiveStageContext/ILifecycleTrackingContext — delete or test), EnsureWhizbangInitializedAsync, UseWhizbangSecurityHeaders, ASB MaxAutoLockRenewalDuration, RabbitMQ MaxChannels/PrefetchCount.
- **2026-08-05 — Phase 8 re-verification sweep COMPLETE** (branch docs/verify-sweep-2026-08): all **209** stale pages verified against library `0bc6065b` by 24 agent chunks; ~95 pages needed fixes. Recurring drift roots: the work-pump decomposition (process_work_batch/ProcessWorkBatchAsync removal), the 077/078 event-body split (wh_event_store is a pointer table), LifecycleStage 24→28, NonBlockingSchemaInit default flip, shipped-but-marked-planned features (read-path upcasting, managed-resource health), and stale "current definition in mig NNN" statements. Phase 6 rode along: mermaid `tests=[…]` annotations added wherever a confidently-covering test existed. A list of ~20 stale library-side XML doc comments / `<tests>` pointers was collected for a lib-side cleanup PR (see sweep commit messages).
- **Remaining open work**: Phase 4 decisions (drafts/proposals/never-written targets — user calls, list below) · Track 2 test-writing (consolidated list above) · Phase 7 sample tagging (2,701 untagged samples — future effort) · residual unannotated diagrams (conceptual flows with no single covering test).
- **Remaining (superseded by the entry above)**: Phase 4 decisions (below) · Phase 5 waves 2+ (638 uncovered symbols, 229 published targets — same investigate→tag pipeline) · Phase 6/7 (97 diagram pins + sample tagging; unblocked per-page as symbol tags land) · Phase 8 (re-verification sweep of 173 stale pages).
